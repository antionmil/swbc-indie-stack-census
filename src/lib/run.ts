...
export async function runCensus(): Promise<RunSummary> {
  ...
  if (prev.length) {
    const p = prev[0].id;
    /* Both directions are guarded on the site having been FETCHED SUCCESSFULLY
       in both runs. Without that guard one 403 reads as a company throwing away
       its entire stack in a week, and the feed — the only reason anybody comes
       back — fills up with fiction. */
    const a = (await db`
      insert into changes (run_id, prev_run_id, domain, tech, kind, evidence)
      select ${runId}, ${p}, f.domain, f.tech, 'added', f.evidence->0->>'detail'
      from findings f
      where f.run_id = ${runId}
        and exists (select 1 from fetches x where x.run_id = ${p} and x.domain = f.domain and x.ok = 1)
        and not exists (select 1 from findings g where g.run_id = ${p} and g.domain = f.domain and g.tech = f.tech)
      returning id`) as { id: number }[];
    const r = (await db`
      insert into changes (run_id, prev_run_id, domain, tech, kind, evidence)
      select ${runId}, ${p}, f.domain, f.tech, 'removed', f.evidence->0->>'detail'
      from findings f
      where f.run_id = ${p}
